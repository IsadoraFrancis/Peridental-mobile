...
import { useFocusEffect, useNavigation } from "@react-navigation/native"; // Importar useFocusEffect
import { NativeStackNavigationProp } from "@react-navigation/native-stack";
import { caseService } from "@/services/caseService";
import { CreateCaseData, AddEvidenceData, AddReportData } from "@/types/case";
...
export function AddCase() {
  ...
  const reportTypeOptions = [
    { label: "laudo_pericial" },
    { label: "relatorio_tecnico" },
    { label: "parecer_odontologico" },
  ];
  ...
  const handleAddOrUpdateEvidence = () => {
    if (
      ...
      Alert.alert(
        "Erro",
        "Por favor, preencha o tipo e a descrição da evidência."
      );
      return;
    }
  ...
  const handleAddOrUpdateReport = () => {
    if (
      !selectedReportType ||
      selectedReportType === "Tipo do Laudo" ||
      !reportDescription
    ) {
      Alert.alert(
        "Erro",
        "Por favor, preencha o tipo e a descrição do laudo/documento."
      );
      return;
    }
    ...
    if (editingReportId) {
      setReports(
        reports.map((rep) => (rep.id === editingReportId ? newReport : rep))
      );
    } else {
  ...
  const handleDeleteReport = (id: string) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Excluir",
          onPress: () => setReports(reports.filter((rep) => rep.id !== id)),
        },
      ]
    );
  };
  ...
  const handleSaveCase = async () => {
    try {
      ...
      const caseData: CreateCaseData = {
        ...
        type: selected1 as "acidente" | "identificacao" | "criminal",
        status: selected3 as "em_andamento" | "finalizado" | "arquivado",
        data: selectedDate || new Date(),
      };

      const response = await caseService.createCase(caseData);

      // Após criar o caso, adicionar as evidências
      for (const evidence of evidences) {
        const evidenceData: AddEvidenceData = {
          ...
          location: {
            latitude: parseFloat(evidence.location.split(",")[0].split(":")[1]),
            longitude: parseFloat(
              evidence.location.split(",")[1].split(":")[1]
            ),
          },
          file: evidence.imageUri
            ? {
      ...
      for (const report of reports) {
        ...
        await caseService.addReport(reportData);
      }
      ...
      // Navegar para a tela de detalhes do caso ou lista de casos
    } catch (error) {
      console.error("Erro ao criar caso:", error);
      Alert.alert("Erro", "Não foi possível criar o caso. Tente novamente.");
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.contentContainer}>
        ...
        {/* Título do Caso */}
        <Text style={styles.caseTitulo}>Título do Caso</Text>
        <TextInput
        ...
        <View style={styles.evidenceSection}>
          ...
          >
            <Text style={styles.evidenceSelectionText}>
              {selectedReportType}
            </Text>
            <AntDesign
              name="down"
          ...
          {/* Adicionar o botão de salvar no final */}
          <TouchableOpacity
